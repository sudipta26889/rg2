...
static const char *WIFI_TAG = "RoomGuardianV2::WiFi";
...
static void wifi_prov_handler(void *user_data, wifi_prov_cb_event_t event, void *event_data)
{
    switch(event)
    {
        ...
        ESP_LOGI(WIFI_TAG,"[WIFI_PROV_START]");
        break;
        case WIFI_PROV_CRED_RECV :
        ESP_LOGI(WIFI_TAG ,"cred : ssid : %s pass : %s",((wifi_sta_config_t*)event_data)->ssid,((wifi_sta_config_t*)event_data)->password);
        break;
        case WIFI_PROV_CRED_SUCCESS :
        ESP_LOGI(WIFI_TAG,"prov success");
        wifi_prov_mgr_stop_provisioning();
        break;
        case WIFI_PROV_CRED_FAIL :
        ESP_LOGE(WIFI_TAG,"credentials worng");
        wifi_prov_mgr_reset_sm_state_on_failure();
        print_qr();
        break;
        case WIFI_PROV_END: 
        ESP_LOGI(WIFI_TAG,"wifi prov ended");
    ...
    if(event_base == WIFI_EVENT)
    {
        switch(event_id)
        {
            ...
            case WIFI_EVENT_STA_DISCONNECTED :
                ESP_LOGE(WIFI_TAG,"ESP32 Disconnected, retrying");
                retry_cnt ++;
                if(retry_cnt < MAX_RETRY_NUM)
                {
                    esp_wifi_connect();
                }
                else ESP_LOGE(WIFI_TAG,"Connection error");
                break;
            default : break;
        }
    }
...
static void prov_start(void)
{
    ...
    if(is_provisioned)
    {
        ESP_LOGI(WIFI_TAG,"Already provisioned");
